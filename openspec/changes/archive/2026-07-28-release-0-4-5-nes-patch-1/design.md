...
`spring-authorization-server-0.4` is a Git repository at `spring-authorization-server-0.4`. This change releases `0.4.5-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.

Representative publications:

`cn.bjca.footstone.bpring.security:bjca-footstone-bpring-security-oauth2-authorization-server:0.4.5-nes.patch.1`

Declared internal dependencies:

`spring-framework-5.3`, `spring-security-5.8`
...
- Publish `0.4.5-nes.patch.1` with no internal `cn.bjca.footstone` SNAPSHOT metadata.
...
- Verify immutable remote assets before creating annotated tag `v0.4.5-nes.patch.1`.
...
Only the manifest owner for `spring-authorization-server-0.4` may write to this repository while the component is being prepared.

### 2. Publish only a complete internal RELEASE dependency graph

Set the project version to `0.4.5-nes.patch.1` and update every internal dependency, parent, imported BOM, and plugin version to its approved RELEASE. Generate the complete local publication set and scan all generated POM/BOM metadata, not only the representative GAVs.
...
test change invalidates it. The operator explicitly authorized not rerunning
the expensive full build and test commands for this release. Generate the
release-specific local publication incrementally:

- Local publication/effective POM: `JAVA_TOOL_OPTIONS=-Dfile.encoding=UTF-8 GRADLE_OPTS='-Xmx3g -Dfile.encoding=UTF-8 -Dorg.gradle.workers.max=3' ./gradlew publishToMavenLocal -x test -x asciidoctor -x javadoc --max-workers=3`

Run representative RELEASE-only consumer validation after local publication. Update `README.adoc`, `doc/SUMMARY.md`, `doc/GAV_MAPPING.md` before creating the dedicated release commit. Build and deploy from that recorded commit without subsequent source changes.
...
- Incremental Nexus RELEASE deploy without `clean`;
- release commit push;
...
After the one authorized deploy, download representative POM and binary assets from Nexus RELEASE, verify version and metadata, calculate checksums, and run the RELEASE-only consumer smoke test. Do not create or push `v0.4.5-nes.patch.1` until the component reaches `nexus-verified`.
...
Create annotated tag `v0.4.5-nes.patch.1` on the exact release commit used for deployment. Verify the remote branch and tag after push.
