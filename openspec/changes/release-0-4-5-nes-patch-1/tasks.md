...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-authorization-server-0.4` in `spring-authorization-server-0.4`.
- [x] 1.2 Acquire the component owner lease and record owner/session/timestamps in the release manifest.
- [x] 1.3 Record branch, `origin` fetch/push URLs, HEAD SHA, active OpenSpec changes, toolchain, current version, and target `0.4.5-nes.patch.1` without exposing credentials.
...
- [x] 1.5 Confirm all declared upstream components are Nexus-verified RELEASEs and record their evidence: `spring-framework-5.3`, `spring-security-5.8`.
...
- [x] 2.1 Set the component version to `0.4.5-nes.patch.1`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring.security:bjca-footstone-bpring-security-oauth2-authorization-server:0.4.5-nes.patch.1`.
...
- [x] 3.2 Record the operator-approved exception not to rerun `./gradlew test`; verify the release diff contains no test changes.
...
- [x] 4.3 Create the dedicated release commit for `spring-authorization-server-0.4` `0.4.5-nes.patch.1`.
...
- [x] 7.1 Confirm `v0.4.5-nes.patch.1` does not already exist locally and the target release commit SHA is unchanged; remote check remains blocked by GitHub connectivity.
- [x] 7.2 Create annotated tag `v0.4.5-nes.patch.1` on the exact release commit only after `nexus-verified`.
