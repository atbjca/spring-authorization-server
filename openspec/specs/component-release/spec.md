...
## Purpose
TBD - created by archiving change release-1-5-8-nes-patch-1. Update Purpose after archive.
## Requirements
### Requirement: Exclusive and safe component preparation

The release process MUST assign one active owner to `spring-authorization-server-1.5` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-authorization-server-1.5`.
...
Every generated RELEASE POM or BOM for `1.5.8-nes.patch.1` MUST use approved RELEASE versions for internal `cn.bjca.footstone` dependencies, parents, imported BOMs, and plugins.
...
The component SHALL have one dedicated release commit containing `1.5.8-nes.patch.1`, internal RELEASE dependency updates, `README.adoc`, `doc/SUMMARY.md`, `doc/GAV_MAPPING.md`, approved exclusions, and no unrelated source changes.
...
- **WHEN** no POM, binary, checksum, metadata, or partial module asset exists for `1.5.8-nes.patch.1`
...
After Nexus verification, the coordinator MUST create annotated tag `v1.5.8-nes.patch.1` on the exact release commit used to build and deploy `1.5.8-nes.patch.1`.
