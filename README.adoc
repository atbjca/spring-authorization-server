...
== NES development line 0.4.5-nes.patch.2-SNAPSHOT

This branch has left the immutable RELEASE `0.4.5-nes.patch.1` and now develops `0.4.5-nes.patch.2-SNAPSHOT`. It consumes the current Spring Security development coordinate `5.8.16-nes.patch.2-SNAPSHOT` for functional verification. Do not deploy this SNAPSHOT, and do not reuse the published patch.1 coordinate. The next SAS RELEASE still requires Security `5.8.16-nes.patch.2` in Nexus `releases` at `http://192.168.131.36:8088/repository/releases`.

Current development coordinate:

* `cn.bjca.footstone.bpring.security:bjca-footstone-bpring-security-oauth2-authorization-server:0.4.5-nes.patch.2-SNAPSHOT` (JAR)

Required internal dependencies for the next SAS RELEASE:
...
The last immutable SAS RELEASE remains `0.4.5-nes.patch.1`, which consumed Security `5.8.16-nes.patch.1`.

There are no publication exclusions. To prepare all publications incrementally in the local Maven repository without cleaning or rerunning tests:
...
Before deployment, scan the generated POM for internal SNAPSHOT references and verify that the target GAV is absent from Nexus RELEASE. The internal dependencies project configures the build platform but does not apply `maven-publish`, so it is not a release publication. Deployment is coordinator-controlled and must run only once from the reviewed release commit.
