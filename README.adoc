...
=== NES RELEASE
The NES-maintained release is `1.5.8-nes.patch.1` and is published to the internal Nexus RELEASE repository under:
...
It requires the verified internal RELEASE BOMs
`cn.bjca.footstone.bpring:bjca-footstone-bpring-framework-bom:6.2.19-nes.patch.1` and
`cn.bjca.footstone.bpring.security:bjca-footstone-bpring-security-bom:6.5.11-nes.patch.1`.
The release has no explicit publication exclusions.

Before deployment, generate the publication locally without cleaning or running tests:
...
Scan every generated POM for internal SNAPSHOT versions and verify the complete target publication set is absent from the Nexus RELEASE repository before the coordinator performs the single deployment.
